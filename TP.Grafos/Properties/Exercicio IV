...
namespace TP.Grafos
{
    ...
    internal static class AgendamentoManutencao
    {
        internal static ResultadoManutencao CalcularAgendamento(IGrafos grafo)
        {
            ...
            int numVertices = grafo.QuantVertices;
            ...
            for (int i = 0; i < arestas.Count; i++)
            {
                Aresta a = arestas[i];
                if (a == null) continue;
                if (a.Origem >= 1 && a.Origem <= numVertices) grau[a.Origem]++;
                if (a.Destino >= 1 && a.Destino <= numVertices) grau[a.Destino]++;
            }
            ...
            {
                int dx = grau[x.Origem] + grau[x.Destino];
                int dy = grau[y.Origem] + grau[y.Destino];
        ...
        private static List<Aresta> ExtrairTodasArestas(IGrafos grafo)
        {
            ...
            for (int origem = 1; origem <= numVertices; origem++)
            {
                ...
                for (int i = 0; i < vizinhos.Count; i++)
                {
                    Aresta a = vizinhos[i];
                    if (a == null) continue;
                    int u = a.Origem < a.Destino ? a.Origem : a.Destino;
                    int v = a.Origem < a.Destino ? a.Destino : a.Origem;
        ...
        public static void ExibirResultados(ResultadoManutencao resultado)
        {
            ...
            for (int i = 0; i < turnos.Count; i++)
            {
                ...
                {
                    Aresta rota = rotas[j];
                    char hubOrigem = (char)('A' + rota.Origem - 1);
                    char hubDestino = (char)('A' + rota.Destino - 1);
        ...
        public static bool VerificarAgendamentoValido(ResultadoManutencao resultado)
        {
            ...
            foreach (KeyValuePair<int, List<Aresta>> kvp in resultado.RotasPorTurno)
            {
                ...
                {
                    Aresta a = lista[i];
                    if (verticesTurno.Contains(a.Origem) || verticesTurno.Contains(a.Destino))
                    {
                        return false;
                    }
                    verticesTurno.Add(a.Origem);
                    verticesTurno.Add(a.Destino);
                }
            }
