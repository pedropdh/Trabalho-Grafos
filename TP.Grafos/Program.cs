        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                Console.WriteLine("===- PERGUNTAS -===\n" +
                    "1 - Qual é o trajeto mais econômico para enviar cargas entre dois centros? (Roteamento de Menor Custo)\n" +
                ...
                {
                    case '1':
                        Console.Write("Informe de qual HUB deseja sair: ");
                        int origem = int.Parse(Console.ReadLine());
                        Console.Write("Informe a qual HUB deseja chegar: ");
                        int destino = int.Parse(Console.ReadLine());
                        ExercicioI x = new ExercicioI();
                        double custoTotal;
                        List<int> caminho = x.CalcularMenorCaminho(grafo, origem, destino, out custoTotal);
                        Console.WriteLine("Menor caminho de " + origem + " até " + destino + ":");
                        Console.WriteLine(caminho == null ? "Não existe caminho." : string.Join(" -> ", caminho));
                    ...
                    case '4':
                        ...
                        break;
                    case '5':
                        ExercicioV ex5 = new ExercicioV(); 
                        Console.WriteLine("===== Cenário A ======\n");
                        if (ex5.TemCicloEuleriano(grafo))
                        {
                            Console.WriteLine("Existe um percuso passando por todas as ROTAS: ");
                            List<int> caminhoEuleriano = ex5.EncontrarCicloEuleriano(grafo, 1);
                            Console.WriteLine(string.Join(" -> ", caminhoEuleriano));
                        }
                        else
                            Console.WriteLine("O grafo não possui ciclo euleriano.\n");

                        Console.WriteLine("===== Cenário B ======\n");
                        List<int> caminhoHamiltoniano = ex5.EncontrarCaminhoHamiltoniano(grafo, 1);
                        if (caminhoHamiltoniano != null)
                        {
                            Console.WriteLine("Existe um percuso passando por todos os HUBS:");
                            Console.WriteLine(string.Join(" -> ", caminhoHamiltoniano));
                        }
                        else
                            Console.WriteLine("Nenhum ciclo foi encontrado.");
                        break;
                    default:
